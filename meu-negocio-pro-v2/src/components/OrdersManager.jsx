...
export function OrdersManager({ inventory, setInventory, pessoas, setPessoas, transactions, setTransactions, addToast, isActive }) {
  ...
  const handleLabelData = useCallback(async (data) => {
    ...
    if (data.customerName) {
      const existing = pessoas.find(p => p.name.toLowerCase() === data.customerName.toLowerCase())
      if (existing) {
        setSelectedPessoa(existing.name)
        addToast(`Cliente "${existing.name}" selecionado!`, 'success')
      } else {
        const novo = { id: generateId(), name: data.customerName.trim(), document: '', role: 'cliente', contact: '' }
        if (setPessoas) setPessoas(prev => [...prev, novo])
        setSelectedPessoa(novo.name)
        await supabase.from('pessoas').insert([novo])
        addToast(`Novo cliente "${novo.name}" cadastrado!`, 'success')
      }
    }
