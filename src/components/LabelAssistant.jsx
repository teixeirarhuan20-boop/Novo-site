...
export function LabelAssistant({ inventory, pessoas, onDataExtracted, addToast }) {
  ...
  const processFile = (file) => {
    ...
    if (file.type.startsWith('image/') && addToast) {
      ...
      reader.onload = (e) => {
        ...
        img.onload = () => {
          ...
          if (avgLum < 45) addToast("🌙 Imagem muito escura! Tente iluminar melhor a etiqueta.", "warning");
    ...
    Tesseract.recognize(
      ...
      'por',
      { logger: m => { 
          if (m.status === 'recognizing text' || m.status === 'loading tesseract core' || m.status.includes('loading')) {
             const progressStr = `Baixando Motor: ${(m.progress * 100).toFixed(0)}%`;
             if (m.status === 'recognizing text') {
               setOCRProgress(`Lendo Imagem: ${(m.progress * 100).toFixed(0)}%`);
             } else {
               setOCRProgress(progressStr);
             }
          } 
      } }
      ...
      <div style={{ display: 'flex', gap: '1rem', width: '100%' }}>
        ...
        >
          ...
          <label htmlFor="label-upload" style={{ cursor: cooldown > 0 ? 'not-allowed' : 'pointer', display: 'flex', flexDirection: 'column', alignItems: 'center', width: '100%', margin: 0 }}>
             <div style={{ fontSize: '2rem', marginBottom: '0.5rem' }}>{cooldown > 0 ? '⏳' : isProcessing ? '⚙️' : '📸'}</div>
             <p style={{ fontSize: '0.9rem', margin: 0, color: cooldown > 0 ? '#ef4444' : '#3b82f6', fontWeight: '600', textAlign: 'center' }}>
               {cooldown > 0 ? `Aguarde ${cooldown}s...` : isProcessing ? OCRProgress || 'Lendo Etiqueta...' : 'Arrastar Imagem/PDF'}
             </p>
          </label>
          {isProcessing && (
